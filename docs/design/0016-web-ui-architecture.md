...
- **Owner:** Nathan Klick
- **Date:** 2026-09-16
- **Summary:** Rackmarshal gains three browser surfaces — the existing `sso` login site, a new
  `portal` for tenant users, and a new `console` for platform administrators — built as
  ...
  the gateway itself, which is what lets the gateway keep CORS off. `cli` gains a loopback
...
- Keep 0007's guarantee that login works without JavaScript except for WebAuthn, and extend WCAG 2.2 AA
  to the new surfaces.
- Give `cli` a login that works where the device grant is awkward, without a second token type.
- Work in air-gapped installations, which rules out loading anything from a public CDN at runtime.

**Non-goals**

- The page inventory of each surface — [0017](0017-portal.md) and [0018](0018-console.md).
...
- Replacing `cli`. The CLI stays the complete operator interface; the portals are additive.
...
Each surface is an OIDC relying party of `identity`, using the authorization code flow with PKCE
...
Through `common`, per CONVENTIONS. Page handlers emit `http.route`, `http.response.status_code`,
...
table in the portal resolves to the `inventory` query behind it.
...
- **Artifacts** — the deployment set CONVENTIONS lists: OCI image, Helm chart, deb/rpm, and an NSIS
  installer. Neither surface needs cgo, so both cross-compile normally, unlike `identity`.
...
- **Session store ownership** — PostgreSQL reuses existing conventions, but it gives the portal and
  console a database they otherwise would not have. `sso` already keeps its sessions in
